"""
    sparsekmeans1permute(x, class, K, sbounds, nperms, silent=false)

Permutation test for SKFR1.

# Input: 
- `x`: data  n by p (the more common format of real data.)
- `class`: initial class, a vector
- `K`: number of classes
- `sbounds`: the range of sparsity levels, an Integer vector
- `nperms`: number of permuted datasets
- `silent`:  boolean for reading out the process
...
function sparsekmeans1permute(x::Matrix{T}, class::Vector{Int}, K::Int, sbounds,
    ...
    for k = 1:nperms
        ...
        iternum=1
        for s in sbounds
            (classout, centerout,selectedvec,WSSval,TSSval) = 
                sparsekmeans1(copy(transpose(permx[:,:,k])), class, K, s)
...
"""
    sparsekmeans2permute(x, class, K, sbounds, nperms, silent=false)
...
function sparsekmeans2permute(x::Matrix{T}, class::Vector{Int}, K::Int, sbounds,
    ...
    for k = 1:nperms
        ...
        for s in sbounds
            (classout, centerout,selectedvec,WSSval,TSSval)= 
                sparsekmeans2(copy(transpose(permx[:,:,k])), class, K, s)
            permtots[iternum,k]=TSSval-sum(WSSval)
            iternum=iternum+1
        end
    end
