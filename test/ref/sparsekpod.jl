...
function assign_clustppSparse(X, k, init_centers,sparsity,
 kmpp_flag= true, max_iter= 20)
  ...
  (clusts, centerout,selectedvec,WSS,obj) = ref_sparsekmeans1(X', init_classes, k, sparsity)
  fit = 1 - (sum(WSS)/obj)
  centers = copy(centerout')
  if kmpp_flag == true
    for iter = 1:max_iter
      classes_kmpp = initclass(copy(X'), k)
      (newclusts, newcenterout,selectedvec,newWSS,newobj) = ref_sparsekmeans1(X', classes_kmpp, k, sparsity)
      if newobj < obj
        obj = newobj
        clusts = newclusts
...
function ref_sparsekpod(X, k, sparsity, kmpp_flag::Bool = true,
  ...
  (clusts, centerout,selectedvec,WSS,obj)= ref_sparsekmeans1(X_copy', init_classes, k, sparsity)
  centers = copy(centerout')
